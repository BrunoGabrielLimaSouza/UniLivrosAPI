...
import org.springframework.http.HttpStatus; // Importado para retorno de erro específico
...
public class AuthController {
    ...
    public ResponseEntity<Void> register(@Valid @RequestBody UsuarioDTO data) {
        try {
            ...
            // Se for BusinessException (400), apenas relança para o ControllerAdvice
            throw e;
        } catch (Exception e) {
            // CAPTURA QUALQUER OUTRA EXCEÇÃO (NullPointer, DB Error, etc.) E IMPRIME O STACK TRACE
            logger.error("ERRO GRAVE NO CADASTRO: ", e);
            // Retorna um 500 interno ou relança RuntimeException que seu ControllerAdvice deve pegar
